...
### Anonymous bounded control-flow profile

The candidate set now has a second decoder-bounded report at
...
with GNU `objdump` 2.42. It profiles the candidate payloads as anonymous
32-bit x86-like byte ranges and keeps linear decoding separate from a
...
|---|---:|
| linear instruction records | 28,594 |
| linear decoded bytes | 80,196 of 80,524 payload bytes |
| invalid/byte-fallback records | 200 |
| direct relative transfers | 4,271 |
| indirect calls | 2,986 |
| conditional-transfer targets at linear instruction starts | 4,232 |
| targets conflicting with a linear instruction boundary | 33 |
| targets outside the originating candidate | 4 |
| recursively reached instructions | 5,638 |

The direct transfers include 39 calls, 1,395 unconditional jumps, and 2,837
conditional jumps. The conservative recursive view reaches only 15,158 bytes
because the compiled payloads use many indirect calls and terminate at
unresolved transfers; this is an observation about the decoder boundary, not
a claim about runtime reachability. A second, explicitly uncertain view
assumes that an indirect `call` returns to the next linear instruction while
leaving indirect `jmp` transfers unresolved:

| Recursive view across 190 candidates | Instructions | Bytes |
|---|---:|---:|
| stop at every indirect transfer | 5,638 | 15,158 |
| assume indirect calls return | 22,694 | 64,470 |

The assumed-return pass traverses 2,555 indirect call sites and is retained as
a coverage control, not as a recovered calling convention; a non-returning
call or a data region decoded as an instruction can inflate it. A 28-case
shifted-start control over four representative candidates changes recursive
coverage substantially, and three exact 66-byte PLL/DXL payload anchors retain
matching normalized transfer profiles. Twenty-four bounded PLL payloads are
included as a structural decoder reference. These controls test decoder
