;;; Controlled canvas-state holdout: request 1024x768 before the trace loads.
...
(with-open-file (marker "C:\\temp\\aaron-canvas-size-loaded.txt"
  ...
  (format marker "BEGIN planning-canvas-size-1024x768~%")
  (format marker "REQUESTED-SIZE 1024 768~%")
  (finish-output marker))

(aaron-set-matching-value "*BUILD-PREMIUM*" t)
(aaron-set-matching-value "SMALL-IMAGE-SCREEN-WIDTH" 1024)
(aaron-set-matching-value "SMALL-IMAGE-SCREEN-HEIGHT" 768)
