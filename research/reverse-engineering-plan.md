...
| 6. Brush and colour pipeline | Early research, first fill writes measured | Seven startup brush profiles and complete ordered perimeter/core masks are captured; `INIT-MAPS` matches private map dimensions, element widths, zero initialization, return value, replacement behavior, and binding restoration. Isolated `BRUSH-STROKE` probes now measure empty/singleton boundaries, direct value propagation, brush-1/2/3/4 two-point footprints, a non-interpolated gapped brush-1 path, first-coordinate-major map indexing, and an aligned CDEX holdout with unchanged isolated map effects. Selection boundaries, overlaps, clipping, SDEX behavior, colour transitions, and brush state remain open. | Brush selection, colour transitions, fill paths, and brush state match captured original calls. |
