...
| 6. Brush and colour pipeline | Early research, first fill writes measured | Seven startup brush profiles and complete ordered perimeter/core masks are captured; `INIT-MAPS` matches private map dimensions, element widths, zero initialization, return value, replacement behavior, and binding restoration. Isolated `BRUSH-STROKE` probes now measure empty/singleton boundaries, direct value propagation, brush-1/2/3 two-point footprints, a non-interpolated gapped brush-1 path, and first-coordinate-major map indexing. Selection boundaries, overlaps, clipping, colour transitions, and brush state remain open. | Brush selection, colour transitions, fill paths, and brush state match captured original calls. |
...
2. Continue isolating `BRUSH-STROKE(PATH VALUE CDEX SDEX)` with the proven
   ...
   vary larger brush IDs, overlaps, clipping, and `CDEX`/`SDEX`; restore every
