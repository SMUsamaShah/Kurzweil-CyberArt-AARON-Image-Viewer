...
(with-open-file (report "C:\\temp\\aaron-brush-stroke-isolated.txt"
                        :direction :output :if-exists :append
                        :if-does-not-exist :create)
  ;; Stage 14 invokes BRUSH-STROKE with brush 1 and a non-adjacent horizontal
  ;; path. This changes only the endpoint gap from the adjacent fixtures.
  ...
  (write-line "STAGE-14-GAPPED-PATH-BEGIN" report)
  (handler-case
      (let* ((owner (find-package "COMMON-GRAPHICS-USER"))
        ...
        (write-line (if (and (eq (symbol-function screen) original-screen)
                             (eq (symbol-function inside) original-inside))
                        "STAGE-14-FUNCTIONS-RESTORED"
                        "STAGE-14-FUNCTIONS-NOT-RESTORED")
                    report)
        (write-line (if (and (eql (boundp brush-symbol) before-brush-bound)
                        ...
                        "STAGE-14-BINDINGS-RESTORED"
                        "STAGE-14-BINDINGS-LEAKED")
                    report))
    (error (problem)
      (declare (ignore problem))
      (write-line "STAGE-14-GAPPED-PATH-ERROR" report)))
  (finish-output report))
...
(with-open-file (report "C:\\temp\\aaron-brush-stroke-isolated.txt"
  ...
  (format report "STAGE-14-GAPPED-PATH~%")
  (format report "END brush-stroke-isolated~%")
  (finish-output report))
