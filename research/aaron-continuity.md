...
(`Implement measured RAN-HAND helper`). The connected GitHub branch reports the
same published content under commit
`0782c0dd9e819e6c9694809de05e1829c5997326`; the connector-created commit has a
different ancestry/SHA, so compare the tree and files rather than assuming the
two commit IDs are identical.
